...
public:
	...
	btJointFeedback* m_jointFeedback[4];
	...
	virtual void resetCamera()
	{
		...
		btTransform target = m_wheels[0]->getRigidBodyA().getWorldTransform();
...
static float maxMotorForce = 1000.f;
...
{
	Hinge2Vehicle* info = (Hinge2Vehicle*)world->getWorldUserInfo();
	btJointFeedback* fb = info->m_jointFeedback[0];
	// printf("fl x %f y %f z %f total %f\n",
	// 	fb[frontLeftWheelIndex].m_appliedTorqueBodyB.x(),
	// 	fb[frontLeftWheelIndex].m_appliedTorqueBodyB.y(),
	// 	fb[frontLeftWheelIndex].m_appliedTorqueBodyB.z(),
	// 	(fb[frontLeftWheelIndex].m_appliedTorqueBodyB.x()*fb[frontLeftWheelIndex].m_appliedTorqueBodyB.x()+fb[frontLeftWheelIndex].m_appliedTorqueBodyB.y()*fb[frontLeftWheelIndex].m_appliedTorqueBodyB.y()+fb[frontLeftWheelIndex].m_appliedTorqueBodyB.x()*fb[frontLeftWheelIndex].m_appliedTorqueBodyB.z()) );
}
...
void Hinge2Vehicle::spawnCar()
{
	...
	const btScalar chassisMass = 5.0f;
	const btScalar wheelMass = 1.f;
	...
	// btCompoundShape* m_wheelShape = new btCompoundShape();
	// uint num_planks = 12;
	// float height = 0.005;
	// for (uint i=0; i<num_planks; i++)
	// {
	// 	float angle = i*180.0/num_planks;
	// 	btCollisionShape* shape = new btBoxShape(btVector3(wheelWidth, height, wheelRadius));
	// 	btTransform localTrans;
	// 	localTrans.setIdentity();
	// 	localTrans.setOrigin(btVector3(0, 0, 0));
	// 	localTrans.setRotation(btQuaternion(sin(angle/2), 0, 0, cos(angle/2)));
	// 	m_wheelShape->addChildShape(localTrans, shape);
	// }
	...
	for (int i = 0; i < 4; i++)
	{
		...
		pBodyB->setContactStiffnessAndDamping(7500., 1000);
		...
		m_jointFeedback[i] = fb;
		pHinge2->setJointFeedback(fb);
		...
		// suspension
		pHinge2->enableSpring(suspensionIndex, true);
		...
		pHinge2->enableSpring(0, true);
		pHinge2->setParam( BT_CONSTRAINT_CFM, cfm, 0 );
		pHinge2->setParam( BT_CONSTRAINT_ERP, erp, 0 );
		pHinge2->setDamping( 0, suspensionDamping, false);
		pHinge2->setStiffness( 0, 1000.f, false);

		btVector3 linLowerLimits(-0.05f,0.f,0.f);
		btVector3 linUpperLimits(0.1f,0.f,0.f);
		...
		btVector3 angLowerLimits(1.f,0.f,0.f);
		btVector3 angUpperLimits(-1.f,0.f,0.f);
		angLowerLimits[steeringIndex-3] = -steeringClamp;
		angUpperLimits[steeringIndex-3] = steeringClamp;
...
void Hinge2Vehicle::spawnBuggie()
{
	...
	tr.setOrigin(btVector3(0, FALLHEIGHT, -3));

	const btScalar chassisMass = 80.0f;
	const btScalar wheelMass = 5.f;
	m_carChassis = localCreateRigidBody(chassisMass, tr, compound);  //chassisShape);
	...
	btVector3 wheelPos[4];
	wheelPos[frontRightWheelIndex] = 	btVector3(	-btScalar(chassisHalfWidth + wheelWidth+.2),	btScalar(FALLHEIGHT-0-chassisHalfHeight),	btScalar(chassisHalfLength-wheelRadius)	-3	)-cog;
	wheelPos[frontLeftWheelIndex] = 	btVector3(	 btScalar(chassisHalfWidth + wheelWidth+.2),	btScalar(FALLHEIGHT-0-chassisHalfHeight),	btScalar(chassisHalfLength-wheelRadius)	-3	)-cog;
	wheelPos[backLeftWheelIndex] = 		btVector3(	 btScalar(chassisHalfWidth + wheelWidth+.2),	btScalar(FALLHEIGHT-0-chassisHalfHeight),	-btScalar(chassisHalfLength-wheelRadius)-3	)-cog;
	wheelPos[backRightWheelIndex] = 	btVector3(	-btScalar(chassisHalfWidth + wheelWidth+.2),	btScalar(FALLHEIGHT-0-chassisHalfHeight),	-btScalar(chassisHalfLength-wheelRadius)-3	)-cog;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		// add constraint to world
		m_dynamicsWorld->addConstraint(pHinge2, true);
		pHinge2->setBreakingImpulseThreshold(INFINITY);
...
void Hinge2Vehicle::setMotorTargets()
{
	...
	if (steer_left)
	{
		...
		gVehicleSteering = std::max(gVehicleSteering,-steeringClamp);
	...
	{
		gVehicleSteering += 0.025;
		gVehicleSteering = std::min(gVehicleSteering,steeringClamp);
	...
	// fric.normalize();
	// static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().setAnisotropicFriction(fric);
	// static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().setAnisotropicFriction(fric);
	// static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().setAnisotropicFriction(fric);
	// static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().setAnisotropicFriction(fric);

	static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().setRollingFriction(.1);
	static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().setRollingFriction(.1);
	static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().setRollingFriction(.1);
	static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().setRollingFriction(.1);

	static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().setSpinningFriction(.02);
	static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().setSpinningFriction(.02);
	static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().setSpinningFriction(.02);
	...
	btScalar wheelRadius = static_cast<btCylinderShape*>(static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().getCollisionShape())->getHalfExtentsWithoutMargin()[0];

	// btScalar currVel = static_cast<btHinge2Constraint*>(m_wheels[0])->getRigidBodyA().getLinearVelocity().length();
	// gTargetForce = (gTargetVelocity - currVel)*maxMotorForce*0.25;
	// gTargetForce = std::max(-400.f, std::min(400.f, (float)gTargetForce));;

	// int idx = frontLeftWheelIndex;
	// btVector3 torque = static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getAngularVelocity().length() > linVel2WheelVel(wheelRadius,maxTargetVelocity) ? btVector3(0.01,0.01,0.01) : ( static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getAngularVelocity()[0] < linVel2WheelVel(wheelRadius,-maxTargetVelocity) ? btVector3(0.01,0.01,0.01) : static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * -gTargetForce );
	// if (torque == btVector3(0.01,0.01,0.01))
	// 	printf("CLIPPING\n");
	// // static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyA().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * gTargetForce );
	// // static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyA().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * gTargetForce );
	// static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyA().applyTorque( torque );
	// static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyA().applyTorque(static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().getAngularVelocity()[0] > linVel2WheelVel(wheelRadius,maxTargetVelocity) ? btVector3(0,0,0) :  static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * -gTargetForce );
	// // static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[backLeftWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * -gTargetForce );
	// // static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[backRightWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * -gTargetForce );
	// static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().getAngularVelocity()[0] > linVel2WheelVel(wheelRadius,maxTargetVelocity) ? btVector3(0,0,0) : static_cast<btHinge2Constraint*>(m_wheels[frontLeftWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * gTargetForce );
	// static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().applyTorque( static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().getAngularVelocity()[0] > linVel2WheelVel(wheelRadius,maxTargetVelocity) ? btVector3(0,0,0) : static_cast<btHinge2Constraint*>(m_wheels[frontRightWheelIndex])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * gTargetForce );

	gTargetVelocity = fabs(gTargetVelocity);
	{
		int idx = frontLeftWheelIndex;
		btVector3 torque;
		if (static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getAngularVelocity().length() > linVel2WheelVel(wheelRadius,gTargetVelocity))
		{
			torque = -btVector3(0.01,0.01,0.01);
		}
		else if (static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getAngularVelocity().length() < linVel2WheelVel(wheelRadius,-gTargetVelocity))
		{
			torque = btVector3(0.01,0.01,0.01);
		}
		else
		{
			torque = static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getWorldTransform().getBasis().getColumn(0) * gTargetForce;
		}
		static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyA().applyTorque( -torque );
		static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().applyTorque( torque );
	}
	{
		int idx = frontRightWheelIndex;
		btVector3 torque;
		if (static_cast<btHinge2Constraint*>(m_wheels[idx])->getRigidBodyB().getAngularVelocity().length() > linVel2WheelVel(wheelRadius,gTargetVelocity))
		{
			torque = -btVector3(0.01,0.01,0.01);
	...
	if (space)
	{
		for (uint i=0; i<4; i++)
		{
			static_cast<btHinge2Constraint*>(m_wheels[i])->getRigidBodyA().applyForce(btVector3(0, ((i==frontLeftWheelIndex || i==frontRightWheelIndex)?900:500), 0), static_cast<btHinge2Constraint*>(m_wheels[i])->getRigidBodyA().getWorldTransform().getOrigin() );
			static_cast<btHinge2Constraint*>(m_wheels[i])->getRigidBodyB().applyForce(btVector3(0,-((i==frontLeftWheelIndex || i==frontRightWheelIndex)?900:500), 0), static_cast<btHinge2Constraint*>(m_wheels[i])->getRigidBodyB().getWorldTransform().getOrigin() );
		}
		space = false;
	}
